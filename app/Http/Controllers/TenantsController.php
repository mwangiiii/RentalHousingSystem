...
class TenantsController extends Controller
{
    ...
    public function property()
    {
        try {
            ...
            if (!$tenantDetails) {
                return redirect()->back()->with('error', 'Tenant details not found.');
            }
            ...
            $fileUrl = Storage::exists($pdfPath) ? Storage::url($pdfPath) : null;
            return view('tenant.property', compact('landlord', 'property', 'payments', 'tenantDetails', 'room', 'maintenanceWorker', 'propertyManager', 'fileUrl'));
        } catch (Exception $e) {
            Log::error('Failed to fetch property details', ['error' => $e->getMessage()]);
            return redirect()->back()->with('error', 'Failed to fetch property details.');
        }
    }
    ...
    private function getUserByRoleAndProperty($roleName, $propertyId)
    {
        try {
            ...
            Log::error('Failed to fetch user by role and property', ['error' => $e->getMessage()]);
            return null;
        }
    }
    ...
    public function payments()
    {
        try {
            ...
            Log::error('Failed to fetch payment reasons', ['error' => $e->getMessage()]);
            return redirect()->back()->with('error', 'Failed to load payment form.');
        }
    }
    ...
    public function maintenance()
    {
        try {
            ...
            Log::error('Failed to load maintenance form', ['error' => $e->getMessage()]);
            return redirect()->back()->with('error', 'Failed to load maintenance form.');
        }
    }
    ...
    public function submitMaintenanceRequest(Request $request)
    {
        try {
            ...
            Log::error('Failed to submit maintenance request', ['error' => $e->getMessage()]);
            return redirect()->back()->with('error', 'Failed to submit maintenance request.');
        }
    }
}